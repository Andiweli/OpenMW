...
if marker_compile not in text:
    ...
    if text.count(needle) != 1:
        raise SystemExit('could not locate unique CMAKE_CXX_EXTENSIONS anchor in OpenMW 0.51 CMakeLists.txt')
...
# experimental-library gate is enabled. Keep the existing Android toolchain
# unchanged during the 0.51 migration and enable the vendor gate explicitly.
if (ANDROID AND CMAKE_CXX_COMPILER_ID MATCHES "Clang")
    ...
    # this exact NDK can compile and link the formatting forms OpenMW uses.
    ...
    if (NOT OPENMW_ANDROID_NDK_R26_STD_FORMAT_WORKS)
        message(FATAL_ERROR
            ...
            "-fexperimental-library and libc++experimental before OpenMW 0.51 can be built.")
    endif()
endif()
'''
...
if marker_link not in text:
    ...
    if text.count(needle) != 1:
        raise SystemExit('could not locate unique openmw target_link_libraries anchor in OpenMW 0.51 apps/openmw/CMakeLists.txt')
    replacement = needle + r'''

    # OPENMW_ANDROID_NDK_R26_EXPERIMENTAL_FORMAT_LINK
    # NDK r26b keeps the gated format implementation's out-of-line support in
...
print('OpenMW 0.51 Android NDK r26 C++20 format compatibility: READY')
