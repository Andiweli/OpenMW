...
foreach ($Required in @($MarkerFile, $JniLib, $SourceRoot, $BuildRoot, $MwShadowTechnique, $ShadowCpp, $ShadowReceiver, $ShadowCaster, $ShadowsBinHpp, $ShadowsBinCpp, $RuntimePatcher, $MainActivity)) {
    if (-not (Test-Path $Required)) {
        throw "Patch 12g requires the working Patch-12f OpenMW 0.51 tree. Missing: $Required"
    }
}

$ExpectedMarker = "OpenMW 0.51.0 Final`ncommit=$FinalCommit"
if ((Read-Lf $MarkerFile).Trim() -ne $ExpectedMarker) {
    throw 'Patch 12g refused a non-0.51-Final runtime payload.'
}

# Require the exact diagnostic baseline we just tested. Patch 12g changes only
# the final main-frustum crop of the Android orthographic directional shadow map.
...
if (-not $MwShadowText.Contains('OPENMW_ANDROID_051_STABLE_ORTHO_SHADOW_BASIS')) {
    throw 'Patch 12g requires Patch 12e stable orthographic shadow basis.'
}
if (-not $ShadowCppText.Contains('OPENMW_ANDROID_051_ORTHOGRAPHIC_SHADOW_MAP')) {
    throw 'Patch 12g requires Patch 12c orthographic shadows.'
}
if (-not $CasterText.Contains('OPENMW_ANDROID_051_GLES2_NATIVE_SHADOW_CLIPPING')) {
    throw 'Patch 12g requires Patch 12d native GLES2 shadow clipping.'
}
if (-not $ReceiverText.Contains('OPENMW_ANDROID_051_GLES2_MANUAL_SHADOW_COMPARE') -or
    ...
    throw 'Patch 12g requires the Patch-12/12b GLES2 receiver baseline.'
}
if (-not $ShadowsBinHppText.Contains('OPENMW_ANDROID_051_SHADOW_GL_DIAG') -or
    ...
    throw 'Patch 12g expects the currently tested Patch 12f diagnostic baseline.'
}
...
# Teach future clean native source extraction the same Patch-12g semantic edit.
...
if (-not $Patcher.Contains('OPENMW_ANDROID_051_ORTHO_NO_MAIN_FRUSTUM_CROP')) {
    $Count = ([regex]::Matches($Patcher, [regex]::Escape($PatcherInsertAnchor))).Count
    if ($Count -ne 1) {
        throw "Patch 12g permanent patcher: expected one final-verification anchor, found $Count"
    }
    $Patcher = $Patcher.Replace($PatcherInsertAnchor, $Patcher12gBlock)
}
...
'@.TrimEnd([char[]]"`r`n")
if (-not $Patcher.Contains('orthographic no-main-frustum-crop marker is missing')) {
    $Count = ([regex]::Matches($Patcher, [regex]::Escape($ReadyPrint))).Count
    if ($Count -ne 1) {
        throw "Patch 12g permanent verification: expected one READY print, found $Count"
    }
...
if (-not $MwShadowText.Contains('OPENMW_ANDROID_051_ORTHO_NO_MAIN_FRUSTUM_CROP')) {
    throw 'Patch 12g source verification failed: no-main-frustum-crop marker missing.'
}
if (-not $MwShadowText.Contains('OPENMW_ANDROID_051_STABLE_ORTHO_SHADOW_BASIS')) {
    throw 'Patch 12g source verification failed: Patch 12e stable basis was lost.'
}
if (-not $PatcherText.Contains('OPENMW_ANDROID_051_ORTHO_NO_MAIN_FRUSTUM_CROP')) {
    throw 'Patch 12g permanent patcher verification failed.'
}
if (-not $MainText.Contains('Patch 12g no-main-frustum-crop GLES2 shadows over Patch 12f')) {
    throw 'Patch 12g Logcat marker update failed.'
}
...
$WindowsHelper = Join-Path $ProjectRoot 'tools\.openmw-051-patch12g-disable-main-frustum-crop.sh'
$WslHelper = "$WslProject/tools/.openmw-051-patch12g-disable-main-frustum-crop.sh"
...
echo
echo 'OpenMW 0.51 Patch 12g source: READY'
...
echo '  ComputeLightSpaceBounds tightening: still ENABLED (isolated A/B)'
...
printf '%s  %s\n' "$JNI_SHA" "$JNI" > "$PROJECT/buildscripts/openmw-051-patch12g-libopenmw.sha256"

echo
echo 'OpenMW 0.51 Patch 12g no-main-frustum-crop native rebuild: SUCCESS'
...
Write-Host ''
Write-Host 'OpenMW 0.51 Patch 12g - Disable Main-Frustum Crop A/B' -ForegroundColor Cyan
Write-Host 'Only the final main-camera-frustum crop is bypassed for Android orthographic directional shadows.' -ForegroundColor Yellow
Write-Host 'ComputeLightSpaceBounds tightening and Patch 12f diagnostics remain enabled.' -ForegroundColor Yellow
...
$MainText = Read-Lf $MainActivity
if (-not $MwShadowText.Contains('OPENMW_ANDROID_051_ORTHO_NO_MAIN_FRUSTUM_CROP')) {
    throw 'Patch 12g final source verification failed.'
}
if (-not $MainText.Contains('Patch 12g no-main-frustum-crop GLES2 shadows over Patch 12f')) {
    throw 'Patch 12g final Logcat marker verification failed.'
}
if (-not (Test-Path $SymbolLib)) {
    throw 'Patch 12g symbol library is missing after rebuild.'
}
$JniSize = (Get-Item $JniLib).Length
$SymbolSize = (Get-Item $SymbolLib).Length
if ($JniSize -ge $SymbolSize) {
    throw "Patch 12g strip verification failed: packaged=$JniSize symbols=$SymbolSize"
}

Write-Host ''
Write-Host 'Patch 12g is ready for APK assembly.' -ForegroundColor Green
