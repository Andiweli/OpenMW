...
function Assert-ContainsAll([string]$Path, [string[]]$RequiredTokens) {
    ...
    foreach ($Token in $RequiredTokens) {
        if (-not $Text.Contains($Token)) {
            throw "Patch 36 validation failed for $Path. Missing: $Token"
        }
    }
}
...
function Assert-Sha256([string]$Path, [string]$Expected) {
    ...
    if ($Actual -ne $Expected.ToLowerInvariant()) {
        throw "Patch 36 refused a modified retained file: $Path expected=$Expected actual=$Actual"
    }
}
...
foreach ($Required in @(
    ...
    if (-not (Test-Path -LiteralPath $Required)) {
        throw "Patch 36 requires the completed Patch-35 project plus the extracted Patch-36 files. Missing: $Required"
    }
}

if ((Read-Lf $MarkerFile).Trim() -ne "OpenMW 0.51.0 Final`ncommit=$FinalCommit") {
    throw 'Patch 36 refused a non-0.51.0-Final runtime payload.'
}
...
if ($ExpectedNativeSha -ne $ActualNativeSha) {
    throw "Patch 36 requires the exact Patch-35 WetWorld native library. expected=$ExpectedNativeSha actual=$ActualNativeSha"
}
...
Write-Host 'OpenMW 0.51 Patch 36 launcher/settings validation: PASS'
Write-Host 'Native rebuild: NO'
Write-Host 'New launcher controls: Controller Trigger Sensitivity; Point Lighting on Groundcover'
Write-Host 'Imported settings: GUI/controller trigger press+release; Groundcover/point lighting'
