...
$RainLens = Join-Path $ProjectRoot 'app\src\main\assets\android_omwfx\rainlens_android_051_v12.omwfx'
...
foreach ($Required in @(
    ...
    if (-not (Test-Path -LiteralPath $Required)) {
        throw "Patch 33 requires the completed Patch-32 project plus the extracted Patch-33 files. Missing: $Required"
    }
}

if ((Read-Lf $MarkerFile).Trim() -ne "OpenMW 0.51.0 Final`ncommit=$FinalCommit") {
    throw 'Patch 33 refused a non-0.51.0-Final runtime payload.'
}
...
if ($ExpectedNativeSha -ne $ActualNativeSha) {
    throw "Patch 33 requires the exact Patch-29 Tex_Depth/CPU-ray libopenmw.so retained by Patch 30. expected=$ExpectedNativeSha actual=$ActualNativeSha"
}
...
foreach ($Need in @(
    ...
    if (-not $PatcherText.Contains($Need)) {
        throw "Patch 33 native base is incomplete: $Need"
    }
}

$MainText = Read-Lf $MainActivity
foreach ($Need in @(
    'OpenMW 0.51 Patch 33 Gate H4c runtime',
    '"godrays_android_051_depthfixed_vivid,lensflare_android_051_rayocc,gateh_bloom051,rainlens_android_051_v12"',
    'val godraysShader = "godrays_android_051_depthfixed_vivid.omwfx"',
    'val rainLensShader = "rainlens_android_051_v12.omwfx"',
    'TEXDEPTH-VIVID-DYNAMIC-GODRAYS051+DIRECT-SUN-GLOW+LENSFLARE051+CPU-RAY-OCCLUSION+BLOOM+WEATHER-RAINLENS-V12-051',
    '"transparent postpass" to if (omwfxGateH4cSelected) "true"',
    'launcher_shader_preset_applied_v33_openmw051_gate_h4c'
)) {
    if (-not $MainText.Contains($Need)) {
        throw "Patch 33 MainActivity is incomplete: $Need"
    }
}

$BlockedStart = $MainText.IndexOf('val gateH4cBlockedShaders = listOf(')
$BlockedEnd = $MainText.IndexOf("`n        )", $BlockedStart)
if ($BlockedStart -lt 0 -or $BlockedEnd -lt 0) {
    throw 'Patch 33 could not resolve the obsolete-shader block.'
}
...
if ($BlockedText.Contains('godrays_android_051_depthfixed_vivid.omwfx') -or
    ...
    $BlockedText.Contains('rainlens_android_051_v12.omwfx')) {
    throw 'Patch 33 active OMWFX technique was accidentally added to the obsolete-shader block.'
}
foreach ($Obsolete in @(
    ...
    if (-not $BlockedText.Contains($Obsolete)) {
        throw "Patch 33 obsolete-shader block is missing: $Obsolete"
    }
}

$GradleText = Read-Lf $BuildGradle
foreach ($Need in @(
    'OpenMW 0.51 Patch 33',
    "file('src/main/assets/android_omwfx/godrays_android_051_depthfixed_vivid.omwfx')",
    "file('src/main/assets/android_omwfx/rainlens_android_051_v12.omwfx')",
    'def gateH4cGodrays =',
    'def gateH4cRainLens =',
    ...
    if (-not $GradleText.Contains($Need)) {
        throw "Patch 33 Gradle gate is incomplete: $Need"
    }
}
...
foreach ($Need in @(
    ...
    if (-not $GodraysText.Contains($Need)) {
        throw "Patch 33 lost the Patch-30 vivid Godrays shader: $Need"
    }
}
...
foreach ($Forbidden in @(
    ...
    if ($GodraysText.Contains($Forbidden)) {
        throw "Patch 33 Godrays shader contains a forbidden stale/static token: $Forbidden"
    }
}
...
foreach ($Need in @(
    ...
    if (-not $LensflareText.Contains($Need)) {
        throw "Patch 33 lost the device-proven Patch-26 Lensflare: $Need"
    }
}
...
foreach ($Need in @(
    ...
    if (-not $BloomText.Contains($Need)) {
        throw "Patch 33 lost the calibrated Patch-20 Bloom: $Need"
    }
}

$RainLensText = Read-Lf $RainLens
foreach ($Need in @(
    "uniform_float rainlens_strength_v33 {`n    default = 0.72;",
    "uniform_float rainlens_refraction_v33 {`n    default = 0.92;",
    "uniform_float rainlens_density_v33 {`n    default = 0.48;",
    ...
    'version = "4.2-051-rainlens-v12";'
)) {
    if (-not $RainLensText.Contains($Need)) {
        throw "Patch 33 RainLens shader is incomplete: $Need"
    }
}
foreach ($Forbidden in @(
    'omw_GetDepth(',
    ...
    if ($RainLensText.Contains($Forbidden)) {
        throw "Patch 33 RainLens contains an unwanted dependency: $Forbidden"
    }
}
...
Write-Host 'OpenMW 0.51 Patch 33 Gate H4c validation: PASS'
Write-Host 'Native rebuild: NO'
Write-Host 'RainLens: exact OpenMW 0.50 v1.2 algorithm, defaults 0.72/0.92/0.48, one large moving-drop population, one refraction tap'
Write-Host 'Runtime chain: godrays_android_051_depthfixed_vivid,lensflare_android_051_rayocc,gateh_bloom051,rainlens_android_051_v12'
