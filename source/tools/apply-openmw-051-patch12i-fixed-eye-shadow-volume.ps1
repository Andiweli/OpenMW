...
foreach ($Required in @($MarkerFile, $JniLib, $SourceRoot, $BuildRoot, $MwShadowTechnique, $ShadowCpp, $ShadowReceiver, $ShadowCaster, $ShadowsBinHpp, $ShadowsBinCpp, $RuntimePatcher, $MainActivity)) {
    if (-not (Test-Path $Required)) {
        throw "Patch 12i requires the working Patch-12h OpenMW 0.51 tree. Missing: $Required"
    }
}

$ExpectedMarker = "OpenMW 0.51.0 Final`ncommit=$FinalCommit"
if ((Read-Lf $MarkerFile).Trim() -ne $ExpectedMarker) {
    throw 'Patch 12i refused a non-0.51-Final runtime payload.'
}
...
if (-not $MwShadowText.Contains('OPENMW_ANDROID_051_ORTHO_NO_MAIN_FRUSTUM_CROP')) {
    throw 'Patch 12i requires the currently tested Patch 12g no-main-frustum-crop baseline.'
}
if (-not $MwShadowText.Contains('OPENMW_ANDROID_051_ORTHO_NO_CASTER_BOUNDS_TIGHTENING')) {
    throw 'Patch 12i requires the currently tested Patch 12h no-caster-bounds-tightening baseline.'
}
...
# ---------------------------------------------------------------------------
# Patch 12i: remove the remaining main-view orientation dependency from the
# BASE orthographic directional-light shadow camera itself.
# ---------------------------------------------------------------------------
$OldDirectionalStart = @'
...
Replace-Exact $MwShadowTechnique 'Patch 12i fixed eye-anchored orthographic shadow volume A/B' $OldDirectionalStart $NewDirectionalStart
...
# Teach future clean native source extraction the same Patch-12i semantic edit.
$Patcher = Read-Lf $RuntimePatcher
$PatcherInsertAnchor12i = '# Final structural verification.'
$Patcher12iBlock = @'
# ---------------------------------------------------------------------------
...
if (-not $Patcher.Contains('OPENMW_ANDROID_051_ORTHO_FIXED_EYE_VOLUME')) {
    $Count = ([regex]::Matches($Patcher, [regex]::Escape($PatcherInsertAnchor12i))).Count
    if ($Count -ne 1) {
        throw "Patch 12i permanent patcher: expected one final-verification anchor, found $Count"
    }
    $Patcher = $Patcher.Replace($PatcherInsertAnchor12i, $Patcher12iBlock)
}
...
if (-not $PatcherText.Contains('OPENMW_ANDROID_051_ORTHO_NO_MAIN_FRUSTUM_CROP') -or
    -not $PatcherText.Contains('OPENMW_ANDROID_051_ORTHO_NO_CASTER_BOUNDS_TIGHTENING') -or
    -not $PatcherText.Contains('OPENMW_ANDROID_051_ORTHO_FIXED_EYE_VOLUME')) {
    throw 'Patch 12i permanent patcher verification failed.'
}
if (-not $MwShadowText.Contains('OPENMW_ANDROID_051_ORTHO_NO_CASTER_BOUNDS_TIGHTENING')) {
...
if (-not $MwShadowText.Contains('OPENMW_ANDROID_051_ORTHO_FIXED_EYE_VOLUME')) {
    throw 'Patch 12i source verification failed: fixed eye-anchored orthographic volume marker missing.'
}
if (-not $MainText.Contains('Patch 12i fixed-eye-volume GLES2 shadows over Patch 12h')) {
    throw 'Patch 12i Logcat marker update failed.'
}
...
$WindowsHelper = Join-Path $ProjectRoot 'tools\.openmw-051-patch12i-fixed-eye-shadow-volume.sh'
$WslHelper = "$WslProject/tools/.openmw-051-patch12i-fixed-eye-shadow-volume.sh"
...
if ! grep -Fq 'OPENMW_ANDROID_051_ORTHO_FIXED_EYE_VOLUME' "$MWST"; then
    echo 'ERROR: Patch-12i fixed eye-anchored orthographic volume marker is missing.' >&2
    exit 35
fi
...
echo
echo 'OpenMW 0.51 Patch 12i source: READY'
...
echo '  Base ortho volume: FIXED SIZE, eye-anchored, independent of look direction'
...
printf '%s  %s\n' "$JNI_SHA" "$JNI" > "$PROJECT/buildscripts/openmw-051-patch12i-libopenmw.sha256"

echo
echo 'OpenMW 0.51 Patch 12i fixed-eye-volume native rebuild: SUCCESS'
...
echo 'Next: rebuild/reinstall the APK normally. Stand still and rotate at the same Vivec positions to test whether shadows still pop with view direction.'
'@
...
Write-Host ''
Write-Host 'OpenMW 0.51 Patch 12i - Fixed Eye-Anchored Shadow Volume A/B' -ForegroundColor Cyan
Write-Host 'Patch 12g/12h bypasses stay active; Patch 12i makes the base orthographic shadow volume rotation-invariant.' -ForegroundColor Yellow
...
$MainText = Read-Lf $MainActivity
if (-not $MwShadowText.Contains('OPENMW_ANDROID_051_ORTHO_NO_MAIN_FRUSTUM_CROP')) {
    throw 'Patch 12i final source verification failed: Patch 12g no-main-frustum-crop marker missing.'
}
if (-not $MwShadowText.Contains('OPENMW_ANDROID_051_ORTHO_NO_CASTER_BOUNDS_TIGHTENING')) {
    throw 'Patch 12i final source verification failed: no-caster-bounds-tightening marker missing.'
}
if (-not $MwShadowText.Contains('OPENMW_ANDROID_051_ORTHO_FIXED_EYE_VOLUME')) {
    throw 'Patch 12i final source verification failed: fixed eye-volume marker missing.'
}
if (-not $MainText.Contains('Patch 12i fixed-eye-volume GLES2 shadows over Patch 12h')) {
    throw 'Patch 12i final Logcat marker verification failed.'
}
if (-not (Test-Path $SymbolLib)) {
    throw 'Patch 12i symbol library is missing after rebuild.'
}
$JniSize = (Get-Item $JniLib).Length
$SymbolSize = (Get-Item $SymbolLib).Length
if ($JniSize -ge $SymbolSize) {
    throw "Patch 12i strip verification failed: packaged=$JniSize symbols=$SymbolSize"
}

Write-Host ''
Write-Host 'Patch 12i is ready for APK assembly.' -ForegroundColor Green
Write-Host "Packaged SHA-256: $((Get-FileHash $JniLib -Algorithm SHA256).Hash.ToLowerInvariant())"
Write-Host 'Stand still at a reproducible Vivec position and rotate through the angles that caused shadow popping; test Match Sunlight to sun ON and OFF.' -ForegroundColor Cyan
