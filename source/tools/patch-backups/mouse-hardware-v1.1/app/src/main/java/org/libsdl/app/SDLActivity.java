...
//            mScreenKeyboardShown = true;
    ...
    // OpenMW Android custom-resolution bridge.
    // Relative pointer-capture deltas must never be scaled.
    public static float scaleAbsoluteMouseX(float x) {
        final int logicalWidth = MainActivity.Companion.getResolutionX();
        if (mSurface == null || logicalWidth <= 0) {
            return x;
        }

        final int viewWidth = mSurface.getWidth();
        if (viewWidth <= 0) {
            return x;
        }

        final float scaled = x * ((float) logicalWidth / (float) viewWidth);
        return Math.max(0.0f, Math.min((float) logicalWidth - 1.0f, scaled));
    }

    public static float scaleAbsoluteMouseY(float y) {
        final int logicalHeight = MainActivity.Companion.getResolutionY();
        if (mSurface == null || logicalHeight <= 0) {
            return y;
        }

        final int viewHeight = mSurface.getHeight();
        if (viewHeight <= 0) {
            return y;
        }

        final float scaled = y * ((float) logicalHeight / (float) viewHeight);
        return Math.max(0.0f, Math.min((float) logicalHeight - 1.0f, scaled));
    }
...
class SDLSurface extends SurfaceView implements SurfaceHolder.Callback,
    ...
    // OPENMW_CHROMEOS_NATIVE_MOUSE_V1: gameplay uses this stock SDL relative path.
